...
async function starts() {
    ...
    const kapankah = ['Hari Lagi','Minggu Lagi','Bulan Lagi','Tahun Lagi']
			...
			const premium = ["6289524664142@c.us","6285235245683@c.us"]
                        const Premium = '6289524664142@s.whatsapp.net','6285235245683@s.whatsapp.net'
			const totalchat = await client.chats.all()
			const ownerNumber = ["6289524664142@c.us"] // replace this with your number
	       ...
	       const pushname = client.chats.get(mek.participant) === undefined ? (client.contacts[mek.key.remoteJid].notify ? client.contacts[mek.key.remoteJid].notify : "Kak") : (client.contacts[mek.participant].notify ? client.contacts[mek.participant].notify : "Kak")
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 NINGA ZHUMAKE 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mention All 〙✪══\n╠➥'+teks+'╚═〘 NINGA ZHUMAKE 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 NINGA ZHUMAKE BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 NINGA ZHUMAKE GROUP BC 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
